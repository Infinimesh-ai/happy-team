...
- 未完成的外部验收项：尚未在物理 Linux arm64、Alpine/musl 或 macOS 机器跑完端到端；macOS LaunchAgent 尚未在真实 macOS 重启后验证；`TEAM_ANTHROPIC_API_KEY` / `TEAM_OPENAI_API_KEY` 使用占位值，未能真实验证 Claude/OpenAI 计费链路；没有可用个人 Claude/Codex OAuth 账号，未能完成"切到 PERSONAL_OAUTH 后实际发起一次个人账号请求"的最终业务验收。
