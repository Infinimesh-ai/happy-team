...
| install_cli | 从企业服务器分发 fork 版 CLI tarball，直接解包到 `~/.happy-team/cli` 并生成 `~/.happy-team/bin/happy` wrapper。M3 代码事实：wrapper 与安装自检都使用 `install_node` 实际选中的 Node runtime；若目标机已有系统 Node≥20，不再误调用未必存在的 `~/.happy-team/bin/node` | |
| enroll | 服务端为目标成员生成一次性 EnrollToken → 远端执行 `happy enroll --server <url> --token <t>` | token 单次有效，日志中脱敏 |
| setup_agents | 按目标成员的 agentAuthMode（§9.5）生成 `~/.happy-team/agent.env`：COMPANY_API 模式写入公司 API key 环境变量，PERSONAL_OAUTH 模式不写 key 并在 job 结果标注"待成员完成一次 OAuth 登录"。M3 代码事实：Claude remote 模式依赖 Happy CLI 内置 `@anthropic-ai/claude-agent-sdk` native binary，不要求目标机全局安装 `claude`；provisioning/manual install 会生成 `~/.happy-team/bin/claude` wrapper 供成员运行 `claude login`，Codex 仍依赖目标机 PATH 上的 `codex` CLI | 缺 agent 且无法安装不算 FAILED，标 warning |
...
- **PERSONAL_OAUTH（成员自选）**：不写入公司 key（注意：`ANTHROPIC_API_KEY` 存在会覆盖订阅登录，因此该模式下必须确保 agent.env 中无对应 key）。成员在该机器上完成一次 `claude` / `codex` 的 OAuth 登录即可——**可以直接通过网页开一个远程会话/终端完成**，无需物理接触机器；目标机没有系统级 `claude` 时可使用 provisioning 生成的 `~/.happy-team/bin/claude login`（来自 CLI artifact 内置 Claude Agent SDK native binary）。OAuth 交互本身无法由系统代劳，这是外部约束。
...
- CLI availability 上报中，Claude 现在按"全局 `claude` 命令存在或 Happy CLI 可解析当前平台的 `@anthropic-ai/claude-agent-sdk` native binary"判定为可用，避免干净 provisioned 机器因没有全局 `claude` 而在新建会话页被过滤掉；Codex/Gemini/OpenClaw 仍沿用各自既有检测。
