...
- Provisioning 与 Manual Command 都按目标机 `uname -s` / `uname -m` 生成 Node artifact URL：`/v1/team/artifacts/node/$platform/$arch`，平台支持 `linux|darwin`、架构支持 `x64|arm64`。compose 默认把 host 侧 `.team-artifacts/node` 只读挂载到 `/opt/happy-team/artifacts/node` 供企业放置非 x64 制品。server 会读取 Node artifact 二进制头，识别 64-bit ELF/Mach-O 的平台与架构；若路径和二进制不匹配，预检标为 action_required，artifact 下载接口返回 503。
