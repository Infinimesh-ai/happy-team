...
- 管理员预检接口 `GET /v1/team/admin/preflight` 返回部署状态、公开 URL、CLI artifact、`linux|darwin` + `x64|arm64` Node artifact 可用性和 Company API key 是否配置；响应只包含状态/路径/大小/布尔值，不返回 `HANDY_MASTER_SECRET`、公司 API key 或 SSH 秘密值。Team Members 的 Administration 分组提供 Deployment Preflight 入口，页面复用既有 `ItemList`/`ItemGroup`/`Item` admin UI。
