...
| install_node | 无 Node≥20 时，下载自包含 Node 二进制到 `~/.happy-team/bin/node`（**不依赖 root 与系统包管理器**）。M3 代码事实：`/v1/team/artifacts/node/:platform/:arch` 支持 `linux|darwin` + `x64|arm64`；Linux x64 默认从 server 镜像自身 Node runtime 分发，其他平台从 `TEAM_NODE_ARTIFACT_DIR` 查找企业自托管制品；artifact 分发前会校验 ELF/Mach-O 二进制头与请求平台架构匹配 | 下载失败给出代理配置提示 |
| install_cli | 从企业服务器分发 fork 版 CLI tarball，直接解包到 `~/.happy-team/cli` 并生成 `~/.happy-team/bin/happy` / `claude` / `codex` wrappers。M3 代码事实：Happy/Codex wrapper 与安装自检都使用 `install_node` 实际选中的 Node runtime；若目标机已有系统 Node≥20，不再误调用未必存在的 `~/.happy-team/bin/node` | |
| enroll | 服务端为目标成员生成一次性 EnrollToken → 远端执行 `happy enroll --server <url> --token <t>` | token 单次有效，日志中脱敏 |
| setup_agents | 按目标成员的 agentAuthMode（§9.5）生成 `~/.happy-team/agent.env`：COMPANY_API 模式写入公司 API key 环境变量，PERSONAL_OAUTH 模式不写 key 并在 job 结果标注"待成员完成一次 OAuth 登录"。M3 代码事实：Claude remote 模式依赖 Happy CLI 内置 `@anthropic-ai/claude-agent-sdk` native binary，Codex remote 模式依赖 CLI artifact 内置 `@openai/codex` native binary；provisioning/manual install 会生成 `~/.happy-team/bin/claude` 与 `~/.happy-team/bin/codex` wrappers 供成员运行 OAuth 登录，不要求目标机全局安装 vendor CLI | 缺 agent 且无法安装不算 FAILED，标 warning |
...
- M3 代码事实：Node artifact 路由与 provisioning/manual command 已支持 `linux|darwin` + `x64|arm64`；Linux x64 可直接分发 server runtime，其他平台需在 `TEAM_NODE_ARTIFACT_DIR`/compose artifact mount 中提供 `node` 二进制。macOS start_daemon 路径已生成用户级 LaunchAgent + `~/.happy-team/launchd-start.sh` wrapper，不需要 root 权限；wrapper source `agent.env`，且 Claude Personal OAuth 模式下会尝试从 `~/.claude/.credentials.json` 导出 `CLAUDE_CODE_OAUTH_TOKEN`，避免把 OAuth token 写入 plist。
- 待补真实验收：物理 Linux arm64 或 macOS 真实机器端到端；macOS launchd 持久化实际重启验证；Alpine/musl 自包含 Node artifact 与端到端 provisioning。
...
- **PERSONAL_OAUTH（成员自选）**：不写入公司 key（注意：`ANTHROPIC_API_KEY` 存在会覆盖订阅登录，因此该模式下必须确保 agent.env 中无对应 key）。成员在该机器上完成一次 `claude` / `codex` 的 OAuth 登录即可——**可以直接通过网页开一个远程会话/终端完成**，无需物理接触机器；目标机没有系统级命令时可使用 provisioning 生成的 `~/.happy-team/bin/claude login` / `~/.happy-team/bin/codex login`（来自 CLI artifact 内置 native binaries）。OAuth 交互本身无法由系统代劳，这是外部约束。
...
- 成员自助接口为 `GET /v1/team/me/agent-auth` 与 `PATCH /v1/team/me/agent-auth`；GET 返回当前模式和每台机器的 agent-auth 应用状态，PATCH 会对该成员机器入队/尝试应用并返回 `agentAuthSync` 汇总。管理员代改仍走 `PATCH /v1/team/admin/users/:id`。
...
- CLI availability 上报中，Claude 现在按"全局 `claude` 命令存在或 Happy CLI 可解析当前平台的 `@anthropic-ai/claude-agent-sdk` native binary"判定为可用，避免干净 provisioned 机器因没有全局 `claude` 而在新建会话页被过滤掉；Codex 在 Team provisioning/manual install 后通过 `~/.happy-team/bin/codex` wrapper 进入 PATH，仍沿用命令检测但不再要求系统级安装；Gemini/OpenClaw 仍沿用各自既有检测。
- Provisioning 与 Manual Command 都按目标机 `uname -s` / `uname -m` 生成 Node artifact URL：`/v1/team/artifacts/node/$platform/$arch`，平台支持 `linux|darwin`、架构支持 `x64|arm64`；Linux musl 会通过 `ldd` 检测并追加 `?libc=musl`。compose 默认把 host 侧 `.team-artifacts/node` 只读挂载到 `/opt/happy-team/artifacts/node` 供企业放置非 x64 制品；musl 目录支持 `linux-x64-musl/node`、`linux-arm64-musl/node` 或 `linux/<arch>/musl/node`。server 会读取 Node artifact 二进制头，识别 64-bit ELF/Mach-O 的平台与架构；Linux musl artifact 还会读取 ELF `DT_NEEDED`，若依赖 `libstdc++`、`libgcc_s` 等外部动态库会标为 action_required。路径和二进制不匹配或 musl artifact 不自包含时，artifact 下载接口返回 503。musl 制品不会 fallback 到 server runtime，避免 Alpine 目标误装 glibc Node。
- 根目录脚本 `pnpm team:node-artifacts` 可从 Node.js 官方 dist 下载并校验 `SHASUMS256.txt`，默认生成 `linux-arm64`、`darwin-arm64`、`darwin-x64` 的 Node `20.20.2` artifact 到 `.team-artifacts/node/<platform>-<arch>/node`，让 macOS/arm64 真实验收前的企业自分发制品准备可重复。
- `pnpm-workspace.yaml` 使用 pnpm `supportedArchitectures` 固定 materialize Linux/macOS x64/arm64（含 glibc/musl）的 optional native binaries；构建 Team CLI artifact 前仍需执行 `pnpm install --force` 并用 Deployment Preflight 确认 `happy-cli.tgz` 实际包含目标平台的 Claude/Codex native binaries。
...
- macOS provisioning 的 `start_daemon` 分支写用户级 `~/Library/LaunchAgents/com.happy-team.daemon.plist` 与 `~/.happy-team/launchd-start.sh`；plist 只执行 wrapper，不保存 `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` / `CLAUDE_CODE_OAUTH_TOKEN`。wrapper source `agent.env` 后执行 `happy daemon start-sync`；若没有 `ANTHROPIC_API_KEY` 且存在 Claude credentials JSON，会从 `~/.claude/.credentials.json` 读取 `claudeAiOauth.accessToken` 等常见字段并导出 `CLAUDE_CODE_OAUTH_TOKEN`，降低 launchd 脱离 GUI Keychain session 时 Claude Personal OAuth 失败的风险。若 SSH 会话中 `launchctl bootstrap gui/$UID` / `launchctl load` 不可用，会退回一次性 detached `happy daemon start` 并在日志标 warning。
...
- Provision Machine 管理页现在支持先保存 SSH 凭据并按目标成员筛选选择；选中已保存凭据时可直接创建 provisioning job，不需要重新输入密码/私钥。列表仍只显示 safe fields（label/host/user/auth type/delete-after-use），删除走既有 `DELETE /v1/team/admin/ssh-credentials/:id`。
- Settings 的 Happy Team 分组现在同时提供 `Change Password` 与 `Team Agent Access` 入口，补齐成员自助改密码入口；改密仍复用 `POST /v1/team/auth/change-password` 与现有 `/team/change-password` 页面。Team Agent Access 页面显示每台机器的 agent-auth 应用状态、目标 Claude/Codex 模式和 pending/failed 汇总。
- 管理员预检接口 `GET /v1/team/admin/preflight` 返回部署状态、公开 URL、CLI artifact、`linux|darwin` + `x64|arm64` Node artifact 可用性（含 Linux x64/arm64 musl 可选项）、CLI tarball 内 Claude Agent SDK（Linux glibc/musl + macOS）与 Codex CLI native binary 覆盖情况和 Company API key 是否配置；响应只包含状态/路径/大小/布尔值，不返回 `HANDY_MASTER_SECRET`、公司 API key 或 SSH 秘密值。Team Members 的 Administration 分组提供 Deployment Preflight 入口，页面复用既有 `ItemList`/`ItemGroup`/`Item` admin UI。
...
- 2026-07-09：真实 Chromium 浏览器验证 Provision Machine 的 SSH 凭据管理补口：填写 host/user/password 后 Save Credential 成功，Saved SSH Credentials 只显示 safe fields（label、host、user、auth type、delete-after-use），测试密码没有出现在页面文本；保存后凭据自动选中且 Start Provisioning 在无密码输入时可用；随后通过 Delete Credential 弹窗删除临时凭据。Postgres 查询确认没有 `ui-credential` 或测试 secret 残留。
- 2026-07-09：真实 Chromium 浏览器验证 Settings 的 Happy Team 分组：页面显示 `Change Password / Update your Team password` 与 `Team Agent Access / Choose company API keys or personal OAuth`；触发 Change Password 行后导航到 `/team/change-password`，页面显示当前密码、新密码、确认新密码与 Update Password 按钮。
- 2026-07-09：新增部署预检接口并跑 focused routes/artifacts Vitest；真实登录 token 调用覆盖 admin 200 / member 403，断言响应包含 `team_cli_artifact`、四类 Node artifact、Company API key 配置状态，且不包含测试 `HANDY_MASTER_SECRET`、Anthropic key、OpenAI key 明文。重建 live compose server 后用管理员 HTTP 登录调用 `/v1/team/admin/preflight` 返回 `warning`：Linux x64 server-runtime、Linux arm64 configured、CLI artifact、公开 URL、Company API key 均 OK，Darwin x64/arm64 artifact 缺失为 warning，响应不含 `sk-ant`/`sk-proj`/管理员密码片段。
- 2026-07-09：真实 Chromium 浏览器验证 Deployment Preflight 管理 UI：admin 登录后 Team Members -> Administration 显示 `Deployment Preflight / Check server readiness before provisioning`；触发该行进入 `/team/admin/preflight`，页面显示 Overall Status = `Needs Attention`、server URL、checked timestamp，以及 Ready 的 Handy Master Secret / Team Public Server URL / Team CLI Artifact / Linux x64 / Linux arm64 / Team Anthropic API Key / Team OpenAI API Key，Darwin x64/arm64 显示 Needs Attention。DOM 文本检查确认不包含 `sk-ant`、`sk-proj` 或管理员密码片段。
...
- 2026-07-09：新增 `GET /v1/team/me/agent-auth` 只读状态接口；成员 Team Agent Access 页进入时会显示机器级 APPLIED/PENDING/FAILED、目标 Claude/Codex 模式和汇总，不再只在保存后的弹窗里临时显示同步结果。routes 测试覆盖 member 读取 pending 机器状态、模式切换后状态目标模式更新，且响应不包含测试 Anthropic/OpenAI key 明文。执行 `pnpm --filter happy-server-self-host typecheck`、`pnpm --filter happy-server-self-host test -- sources/team/routes.spec.ts --reporter=dot`、`pnpm --filter happy-app typecheck` 均通过。
- 2026-07-09：补强 CLI artifact 与 Claude remote 可用性：`detectCLIAvailability().claude` 现在把当前平台可解析的 Happy CLI 内置 Claude Agent SDK native binary 视为可用，干净 provisioned 机器即使没有全局 `claude` 也不会在新建会话页过滤掉 Claude；Deployment Preflight 会枚举 `happy-cli.tgz`，检查 Linux/macOS x64/arm64 的 Claude Agent SDK native binary 是否随 CLI 自分发。Docker 构建与源码部署文档要求 `pnpm install --force` 以包含非构建机平台 optional binaries；provisioning 的 `install_cli` 自检改用实际选中的 Node runtime，兼容目标机已有系统 Node≥20。执行 `pnpm --filter happy exec vitest run --project unit src/utils/detectCLI.test.ts --reporter=dot`、`pnpm --filter happy typecheck`、`pnpm --filter happy-server-self-host typecheck`、`pnpm --filter happy-server-self-host test -- sources/team/artifacts.spec.ts sources/team/provision/runner.spec.ts --reporter=dot`、`pnpm --filter happy-server-self-host test -- sources/team/routes.spec.ts --reporter=dot` 均通过。
- 2026-07-09：Provisioning 与 Manual Command 现在都会生成 `~/.happy-team/bin/claude` wrapper，按目标平台/libc 跳转到 CLI artifact 内置的 Claude Agent SDK native binary。这样目标机没有系统级 `claude` 时，成员切到 Claude PERSONAL_OAUTH 后仍可通过远程会话/终端运行 `~/.happy-team/bin/claude login` 完成一次个人登录；wrapper 缺少对应 native binary 时会提示运行 Deployment Preflight 并用 optional dependencies 重建 `happy-cli.tgz`。执行 `pnpm --filter happy-server-self-host typecheck`、`pnpm --filter happy-server-self-host test -- sources/team/artifacts.spec.ts sources/team/provision/runner.spec.ts --reporter=dot` 通过（Vitest 依赖收集共 13 个文件 / 87 tests）。
- 2026-07-09：补齐 Codex 自分发路径：`happy` 包新增 production dependency `@openai/codex@0.130.0`，provisioning/manual install 生成 `~/.happy-team/bin/codex` wrapper，用 install_node 实际选中的 Node runtime 执行 CLI artifact 内置 Codex launcher；Deployment Preflight 现在检查 Linux/macOS x64/arm64 的 Codex native binary 与 launcher 是否在 `happy-cli.tgz` 内。这样干净目标机无需全局 `codex` 或外网安装即可在 COMPANY_API 模式启动 Codex，也可在 PERSONAL_OAUTH 模式运行 `~/.happy-team/bin/codex login`。执行 `pnpm --filter happy-server-self-host typecheck`、`pnpm --filter happy-server-self-host test -- sources/team/artifacts.spec.ts sources/team/provision/runner.spec.ts sources/team/routes.spec.ts --reporter=dot` 通过（Vitest 依赖收集共 13 个文件 / 89 tests）；`pnpm --filter happy typecheck`、`pnpm --filter happy build`、`pnpm install --frozen-lockfile --lockfile-only` 通过；新增 `supportedArchitectures` 后执行 `pnpm install --force`，本地 `node_modules` 已 materialize Linux/macOS x64/arm64 的 Claude/Codex optional packages；临时 `pnpm --filter happy deploy --prod --legacy` 的 Linux x64 artifact 中确认存在 `@openai/codex/bin/codex.js`、`@openai/codex-linux-x64/.../codex` 与 Claude SDK binary，并用生成的 `~/.happy-team/bin/codex` wrapper 跑通 `codex-cli 0.130.0`。
- 2026-07-09：补强 Claude musl 预检：`GET /v1/team/admin/preflight` 现在分别检查 `claude_sdk_binary_linux_x64_glibc`、`claude_sdk_binary_linux_x64_musl`、`claude_sdk_binary_linux_arm64_glibc`、`claude_sdk_binary_linux_arm64_musl`，避免 wrapper 在 musl 目标机选择 `*-musl` package 时出现预检漏报。执行 `pnpm --filter happy-server-self-host typecheck`、`pnpm --filter happy-server-self-host test -- sources/team/artifacts.spec.ts sources/team/routes.spec.ts --reporter=dot` 通过（Vitest 依赖收集共 13 个文件 / 89 tests）。
- 2026-07-09：补齐 musl Node artifact 分发链路：`getTeamNodeArtifactInfo(platform, arch, "musl")` 查找 `linux-<arch>-musl/node` 与 `linux/<arch>/musl/node`，不会 fallback 到 server `process.execPath`；Provisioning 与 Manual Command 在 Linux 目标机上用 `ldd` 检测 musl 并请求 `?libc=musl`；预检新增 `node_artifact_linux_x64_musl` 与 `node_artifact_linux_arm64_musl` warning 项。执行 `pnpm --filter happy-server-self-host typecheck`、`pnpm --filter happy-server-self-host test -- sources/team/artifacts.spec.ts sources/team/routes.spec.ts --reporter=dot` 通过（Vitest 依赖收集共 13 个文件 / 91 tests）。随后补强 ELF `DT_NEEDED` 校验，Linux musl artifact 若依赖 `libstdc++`/`libgcc_s` 会被标为 invalid 并由下载接口拒绝；再次执行同一组 typecheck/Vitest 通过（13 个文件 / 92 tests）。真实 Alpine/musl 机器端到端尚未执行。
- 2026-07-09：尝试 Alpine/musl artifact 实机前置验证：下载 `node-v20.20.2-linux-x64-musl.tar.xz` 并用 `SHASUMS256.txt` 校验通过；提取出的 `node` 在 glibc 宿主机因缺 `/lib/ld-musl-x86_64.so.1` 无法执行，在 `postgres:18-alpine`（已有 `libstdc++`/`libgcc_s`）中可运行并输出 `linux x64 v20.20.2`；在 `python:3.12-alpine`、`golang:1.25-alpine`、`lscr.io/linuxserver/openssh-server` 中因缺 `libstdc++.so.6`/`libgcc_s.so.1` 失败。因此该 unofficial musl tarball 不能证明"干净 Alpine 无 root 依赖"验收成立；当前 server guard 会拒绝这类外部动态库依赖，仍需企业提供真正自包含的 musl Node artifact 后再跑 provisioning E2E。
- 2026-07-09（业主决策，关闭 musl 阻塞项）：目标机环境确定为 **Ubuntu 24.04 / 26.04 x64（glibc）且具备 root 权限**，团队不存在 Alpine/musl 目标机。musl Node artifact 不再是验收阻塞项：`node_artifact_linux_*_musl`、`claude_sdk_binary_linux_*_musl` 等预检 warning 项允许长期为空，"干净 Alpine 无 root" E2E 不再要求执行。已实现的 ldd libc 检测、`?libc=musl` 分发链路与 ELF `DT_NEEDED` 守卫**全部保留**，将来若出现 musl 目标机，只需用 `--partly-static`（静态链接 libstdc++/libgcc、仅保留 musl libc 动态依赖，恰好通过现有守卫且不破坏原生插件 dlopen）在 Alpine 容器内构建一次 Node 并放入 `linux-<arch>-musl/node` 即可，无需改代码。
- 未完成的外部验收项（由业主手动执行）：尚未在物理 Linux arm64 或 macOS 机器跑完端到端（目标机已确定为 Ubuntu x64，此两项降为低优先级）；macOS LaunchAgent 尚未在真实 macOS 重启后验证；`TEAM_ANTHROPIC_API_KEY` / `TEAM_OPENAI_API_KEY` 使用占位值，未能真实验证 Claude/OpenAI 计费链路；没有可用个人 Claude/Codex OAuth 账号，未能完成"切到 PERSONAL_OAUTH 后实际发起一次个人账号请求"的最终业务验收。
