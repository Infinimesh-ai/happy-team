...
| setup_agents | 按目标成员的 agentAuthMode（§9.5）生成 `~/.happy-team/agent.env`：COMPANY_API 模式写入公司 API key 环境变量，PERSONAL_OAUTH 模式不写 key 并在 job 结果标注"待成员完成一次 OAuth 登录"。M3 代码事实：Claude remote 模式依赖 Happy CLI 内置 `@anthropic-ai/claude-agent-sdk` native binary，不要求目标机全局安装 `claude`；Codex 仍依赖目标机 PATH 上的 `codex` CLI | 缺 agent 且无法安装不算 FAILED，标 warning |
