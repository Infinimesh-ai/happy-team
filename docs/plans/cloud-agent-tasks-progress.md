...
- [ ] C1.2 内部意图端点 `POST /v1/team/tasks/:id/intent`（task-token 鉴权）→ 转交状态机；每个意图（含被拒）全量落写 `TeamTaskTransition`
- [ ] C1.3 状态机扩展：`complete_stage`（verify verdict 预留）/`report_blocker`→ESCALATED/`get_task_context`；`WAITING_APPROVAL` 态 + 审批边
- [ ] C1.4 `happy task-mcp` stdio 子命令（get_task_context/complete_stage/report_blocker）经 daemon 转发 server；spawn 注入 `HAPPY_TASK_ID`/`HAPPY_TASK_TOKEN` + 两 agent 的 MCP 配置
